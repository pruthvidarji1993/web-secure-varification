...
exports.runSupplyChainScanner = runSupplyChainScanner;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
// Patterns in postinstall scripts that indicate high-risk behaviour
const DANGEROUS_SCRIPT_PATTERN = /\b(?:curl|wget|bash|sh|powershell|pwsh|python|python3|node\s+-e|exec|eval)\b|https?:\/\//i;
...
function checkPostinstallScripts(projectPath, allDeps) {
    const issues = [];
    ...
    for (const pkgName of Object.keys(allDeps)) {
        ...
        for (const hook of INSTALL_HOOKS) {
            ...
            issues.push({
                ...
                description: `The package "${pkgName}" has a "${hook}" script that runs automatically during npm install.\n` +
                ...
                fixable: false,
                fix: `Audit the ${hook} script in "${pkgName}". If the script is unexpected or suspicious, ` +
...
function checkTyposquatting(projectPath, allDeps) {
    const issues = [];
    const nodeModulesPath = path.join(projectPath, 'node_modules');
    for (const pkgName of Object.keys(allDeps)) {
        ...
        if (isMaliciousByName || isMaliciousByVersion) {
            issues.push({
                ...
                description: `The package "${pkgName}"${isMaliciousByVersion ? ` at version ${version}` : ''} is on the ` +
                ...
                scanner: 'supply-chain',
                fixable: false,
                fix: `Immediately remove "${pkgName}" from your project. Run: npm uninstall ${pkgName}. ` +
        ...
        if (realPkg) {
            issues.push({
                ...
                description: `The package "${pkgName}" closely resembles the popular package "${realPkg}". ` +
                ...
                scanner: 'supply-chain',
                fixable: false,
                fix: `Verify whether you intended to install "${pkgName}" or "${realPkg}". ` +
...
async function runSupplyChainScanner(projectPath) {
