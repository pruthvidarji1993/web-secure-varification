...
exports.runHydrationScanner = runHydrationScanner;
...
function isInsideSafeHook(lines, lineNumber) {
...
const HYDRATION_CHECKS = [
    {
        ...
        description: 'Accessing window, document, or navigator during render causes hydration mismatches because ' +
        ...
        severity: 'high',
        fix: 'Wrap browser API access in a useEffect hook to ensure it only runs on the client. ' +
    ...
    },
    {
        ...
        description: 'Calling new Date() during render returns different timestamps on the server and client, ' +
            'because they execute at different times. This causes React hydration errors in production.',
        severity: 'medium',
        fix: 'Move new Date() inside a useEffect hook to generate the value only on the client. ' +
    ...
    },
    {
        ...
        description: 'Calling Math.random() during render produces different values on the server and client, ' +
            'causing hydration mismatches. Each render pass yields a different random number.',
        severity: 'medium',
        fix: 'Move Math.random() inside useEffect, or generate the random value on the server and pass it as a prop. ' +
    ...
    },
    {
        ...
        description: 'localStorage and sessionStorage are not available on the server (Node.js environment). ' +
            'Accessing them during render causes a ReferenceError on the server and hydration mismatches on the client.',
        severity: 'high',
        fix: 'Wrap all localStorage/sessionStorage access in useEffect to ensure it only runs client-side. ' +
    ...
    },
    {
        ...
        description: 'Using typeof window to conditionally render content causes hydration mismatches. ' +
        ...
        severity: 'low',
        fix: 'Use a state variable initialized in useEffect to track client-side mounting. ' +
...
function scanFileForHydrationIssues(filePath, projectPath) {
    const issues = [];
    let content;
    try {
    ...
    for (const check of HYDRATION_CHECKS) {
        ...
        if (check.requiresJsx && !fileHasJsx)
            continue;
        ...
        while ((match = regex.exec(content)) !== null) {
            ...
            if (check.requiresOutsideHook && isInsideSafeHook(lines, lineNumber))
                continue;
            issues.push({
...
async function runHydrationScanner(projectPath) {
