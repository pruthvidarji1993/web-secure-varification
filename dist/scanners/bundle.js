...
exports.runBundleScanner = runBundleScanner;
...
const BUNDLE_CHECKS = [
    {
        ...
        description: 'Importing the entire lodash library adds approximately 70KB (minified) to your bundle. ' +
            'Most applications only use a small subset of lodash functions.',
        severity: 'high',
        fix: "Import only the specific lodash functions you need. For example, instead of:\n" +
    ...
    },
    {
        ...
        description: 'moment.js adds approximately 232KB (minified) to your bundle and is no longer actively maintained. ' +
            'It is not tree-shakeable, so the entire library is included even if you only use one function.',
        severity: 'high',
        fix: "Replace moment.js with a lighter alternative:\n" +
    ...
    },
    {
        ...
        pattern: /^import\s+\*\s+as\s+\w+\s+from\s+['"](?:rxjs|d3|three|antd|@mui\/material|@material-ui\/core)['"]/gm,
        title: 'Wildcard import from large library',
        description: 'Importing everything from a large library (rxjs, d3, three.js, antd, MUI) prevents tree-shaking ' +
            'and forces the bundler to include the entire library in your bundle.',
        severity: 'medium',
        fix: "Import only the specific submodules or exports you need. For example:\n" +
    ...
    },
    {
        ...
        description: 'Using synchronous require() in React component files prevents code splitting and lazy loading. ' +
            "It forces the module to be included in the initial bundle, increasing the page's load time.",
        severity: 'medium',
        fix: "Replace require() with dynamic import() for code splitting:\n" +
...
function checkHeavyPackageLazyLoading(content, relativePath) {
    const issues = [];
    ...
    const hasLazyLoad = /dynamic\s*\(/.test(content) ||
    ...
    for (const pkg of HEAVY_PACKAGES) {
        ...
        const importPattern = new RegExp(`^import\\s+.+\\s+from\\s+['"]${escapedPkg}['"]`, 'gm');
        let match;
        while ((match = importPattern.exec(content)) !== null) {
            ...
            issues.push({
                ...
                description: `The package "${pkg}" is a large library that significantly increases bundle size. ` +
                ...
                file: relativePath,
                line: lineNumber,
                fix: `Lazy-load "${pkg}" to reduce initial bundle size:\n` +
...
function scanFileForBundleIssues(filePath, projectPath) {
    const issues = [];
    let content;
    try {
    ...
    for (const check of BUNDLE_CHECKS) {
        ...
        while ((match = regex.exec(content)) !== null) {
            ...
            if (check.id === 'sync-require-in-component') {
                ...
                if (!/<[A-Z][a-zA-Z]*[\s/>]|<[a-z]+[\s/>]/.test(content))
                    continue;
            }
            issues.push({
...
async function runBundleScanner(projectPath) {
