...
exports.runLicenseScanner = runLicenseScanner;
...
function detectLicenseFromText(text) {
    if (/GNU GENERAL PUBLIC LICENSE\s+Version 3/i.test(text))
        return 'GPL-3.0';
    if (/GNU GENERAL PUBLIC LICENSE\s+Version 2/i.test(text))
        return 'GPL-2.0';
    if (/GNU AFFERO GENERAL PUBLIC LICENSE/i.test(text))
        return 'AGPL-3.0';
    if (/GNU LESSER GENERAL PUBLIC LICENSE\s+Version 3/i.test(text))
        return 'LGPL-3.0';
    if (/GNU LESSER GENERAL PUBLIC LICENSE\s+Version 2\.1/i.test(text))
        return 'LGPL-2.1';
    if (/Mozilla Public License\s+Version 2\.0/i.test(text))
        return 'MPL-2.0';
    if (/MIT License/i.test(text) || /Permission is hereby granted.*MIT/i.test(text))
        return 'MIT';
    if (/Apache License\s+Version 2\.0/i.test(text))
        return 'Apache-2.0';
    if (/ISC License/i.test(text))
        return 'ISC';
    if (/BSD 2-Clause/i.test(text))
        return 'BSD-2-Clause';
    if (/BSD 3-Clause/i.test(text))
        return 'BSD-3-Clause';
    return null;
}
function getLicenseForPackage(pkgDir) {
...
async function runLicenseScanner(projectPath) {
        ...
        for (const pkgName of Object.keys(allDeps)) {
            ...
            if (FORBIDDEN_LICENSES.has(license)) {
                issues.push({
                    ...
                    description: `The package "${pkgName}" is licensed under ${license}, which is a copyleft license that ` +
                    ...
                    severity: 'critical',
                    scanner: 'license',
                    fix: `Replace "${pkgName}" with a package that has a permissive license (MIT, Apache-2.0, ISC, BSD), ` +
                ...
                issues.push({
                    id: `license-${pkgName}-${license}`,
                    title: `Restrictive license: ${pkgName} (${license})`,
                    description: `The package "${pkgName}" uses the ${license} license, a weak copyleft license. ` +
                    ...
                    severity: 'high',
                    scanner: 'license',
                    fix: `Review whether your use of "${pkgName}" triggers copyleft obligations. ` +
                ...
                issues.push({
                    ...
                    description: `The license for "${pkgName}" could not be determined. Using software with an unknown license ` +
                        `may create legal risks, as the terms of use are unclear.`,
                    severity: 'medium',
                    scanner: 'license',
                    fix: `Check the npm page or repository for "${pkgName}" to determine its license. ` +
                ...
                issues.push({
                    ...
                    description: `The package "${pkgName}" uses the "${license}" license, which is not in the known permissive ` +
