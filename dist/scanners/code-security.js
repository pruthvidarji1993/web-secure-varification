...
exports.runCodeSecurityScanner = runCodeSecurityScanner;
...
const CODE_PATTERNS = [
    {
        ...
        pattern: /\beval\s*\(/g,
        severity: 'critical',
        description: 'eval() executes arbitrary code and is a major security risk. It can execute malicious code if user input is passed to it.',
    ...
    },
    {
        ...
        severity: 'high',
        description: 'new Function() is similar to eval() and can execute arbitrary code. This is dangerous if user input influences the function body.',
    ...
    },
    {
        ...
        severity: 'high',
        description: 'dangerouslySetInnerHTML can introduce XSS vulnerabilities if the content is not properly sanitized.',
    ...
    },
    {
        ...
        severity: 'high',
        description: 'Direct innerHTML assignment can introduce XSS vulnerabilities if the content includes unsanitized user input.',
    ...
    },
    {
        ...
        severity: 'medium',
        description: 'document.write() can introduce XSS vulnerabilities and overwrites the entire document when called after page load.',
    ...
    },
    {
        ...
        severity: 'high',
        description: 'Using exec() with string concatenation or template literals containing variables can lead to command injection.',
    ...
    },
    {
        ...
        severity: 'high',
        description: '__dangerouslyDisableSanitizers disables built-in security sanitization and can introduce XSS vulnerabilities.',
    ...
    },
    {
        ...
        pattern: /\/\/\s*eslint-disable(?:-next-line)?\s+(?:security\/|no-eval|no-implied-eval|no-new-func)/g,
        severity: 'low',
        description: 'An ESLint security rule has been disabled. This may indicate a security issue that has been suppressed instead of fixed.',
    ...
    },
    {
        ...
        pattern: /Math\.random\(\)[^;]*(?:token|secret|id|key|nonce|csrf|session|auth|password|salt)/gi,
        severity: 'medium',
        description: 'Math.random() is not cryptographically secure and should not be used for security-sensitive values like tokens, IDs, or secrets.',
    ...
    },
    {
        ...
        severity: 'low',
        description: 'Hardcoded HTTP URL found. HTTP traffic is not encrypted and can be intercepted (man-in-the-middle attacks).',
        fix: 'Use HTTPS instead of HTTP for all external URLs.',
    },
];
function isInNonCodeContext(line, matchIndex) {
    let inSingle = false;
    let inDouble = false;
...
function scanFileForCodeIssues(filePath, projectPath) {
    const issues = [];
    let content;
    try {
...
async function runCodeSecurityScanner(projectPath) {
