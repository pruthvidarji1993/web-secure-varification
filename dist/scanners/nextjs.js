...
exports.runNextjsScanner = runNextjsScanner;
...
function checkNextConfig(projectPath) {
    const issues = [];
    ...
    if (/dangerouslyAllowSVG\s*:\s*true/.test(configContent)) {
        if (!/contentSecurityPolicy/.test(configContent)) {
            issues.push({
                ...
                description: 'dangerouslyAllowSVG allows SVG files to be served as images, which can contain scripts. Without contentSecurityPolicy, this can lead to XSS vulnerabilities.',
                severity: 'high',
                scanner: 'nextjs',
                file: relativeConfigPath,
...
async function checkNextPublicEnvVars(projectPath) {
    const issues = [];
    // Find all .env files
...
async function checkApiRoutesForAuth(projectPath) {
    const issues = [];
    // Find pages/api/ routes (Next.js pages router)
    const apiFiles = await (0, glob_1.glob)('pages/api/**/*.{js,ts}', {
        cwd: projectPath,
        absolute: true,
        ignore: ['**/node_modules/**'],
    });
    // Also check app/api/ routes (Next.js app router)
    const appApiFiles = await (0, glob_1.glob)('app/**/route.{js,ts}', {
        cwd: projectPath,
        absolute: true,
        ignore: ['**/node_modules/**'],
    });
...
function checkNextAuthConfig(projectPath) {
    const issues = [];
    ...
    if (!hasNextAuthSecret) {
        issues.push({
            ...
            description: 'next-auth is installed but NEXTAUTH_SECRET environment variable is not configured. Without a secret, JWT tokens may use a weak default, compromising session security.',
            severity: 'high',
            scanner: 'nextjs',
...
async function runNextjsScanner(projectPath) {
